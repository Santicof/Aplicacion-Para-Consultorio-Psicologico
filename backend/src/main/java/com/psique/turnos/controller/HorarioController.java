...
public class HorarioController {
    ...
    public ResponseEntity<Map<String, List<String>>> obtenerDisponibilidad(
        ...
        if (horariosProfesional != null && !horariosProfesional.isEmpty()) {
            // Buscar si el profesional trabaja este día
            ...
            // Extraer horas de inicio y fin del horario del profesional
            // Formato: "LUNES:09:00-18:00"
            try {
                String horaParte = horarioDelDia.substring(diaEspanol.length() + 1); // "09:00-18:00"
                String[] partes = horaParte.split("-");
                horaInicio = Integer.parseInt(partes[0].split(":")[0]);
                horaFin = Integer.parseInt(partes[1].split(":")[0]);
            } catch (Exception e) {
                // Si hay error en el parseo, usar horario por defecto
                horaInicio = 8;
                horaFin = 19;
            }
        }
        ...
        // Generar franja horaria según horario del profesional
